...
class RobotLocalizationNode(Node):
    ...
    def lidar_callback(self, msg):
        ...
        if self.prev_scan is not None:
            ...
            pose_msg = Pose2D()
            pose_msg.x = self.pose[0]
            pose_msg.y = self.pose[1]
            pose_msg.theta = self.pose[2]
            self.pose_publisher.publish(pose_msg)
            
            self.get_logger().info(f"Published Pose: x={self.pose[0]:.2f}, y={self.pose[1]:.2f}, theta={self.pose[2]:.2f}")
