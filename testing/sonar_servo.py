...
def grab_bottle():
    send_command('G')
    print("Grabbing the bottle with the servo.")
        ...
        if state == State.SEARCH_GREEN:
            ...
            if green_detected:
                ...
                if contours:
                    ...
                    # Move to the GRAB_BOTTLE state if the green object is close enough
                    distance_front = float(arduino.readline().decode().strip())
                    if distance_front < 10:
                        state = State.GRAB_BOTTLE
        elif state == State.GRAB_BOTTLE:
            grab_bottle()
